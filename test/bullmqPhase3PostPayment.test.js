...
test("post-payment repair scheduler registers only in the worker and respects its flag", async () => {
    ...
    assert.deepEqual(api, { reservation: false, billing: false, postPayment: false });
    assert.deepEqual(worker, { reservation: false, billing: false, postPayment: true });
    assert.deepEqual(disabled, { reservation: false, billing: false, postPayment: false });
