...
export async function barOrders(req, res) {
  try {
    ...
    const orders = rawOrders.map((order) => {
      ...
      const items = dto.items.filter((item) => item.fulfillmentStation === "bar")
...
export async function updateBarFulfillment(req, res) {
  try {
    const businessId = req.session?.user?.businessId
    if (!businessId) return res.status(401).json({ error: "Unauthorized" })
    ...
    return res.json({
      ...
      stationStatus: deriveStationStatus(
        result.order.items.filter((item) => item.fulfillmentStation === "bar"),
      ),
      changed: result.changed,
      updatedAt: result.order.updatedAt,
      order: toStationOrderDTO(result.order, "bar"),
    })
  } catch (err) {
    console.error("[updateBarFulfillment]", err)
    if (Number.isInteger(err?.statusCode)) {
      return res.status(err.statusCode).json({ error: err.message, code: err.code })
    }
    return res.status(500).json({ error: "Failed to update bar fulfilment" })
  }
}
